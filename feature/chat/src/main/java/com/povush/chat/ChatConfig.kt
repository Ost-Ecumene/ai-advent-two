...
object ChatConfig {
    ...
    const val STOP_SEQUENCE = "<END_OF_RESPONSE>"
    val DEFAULT_STOP_SEQUENCES = listOf(STOP_SEQUENCE)
    const val FIRST_MESSAGE = "Я - Квестогенное Ядро версии 0.1. Чем я могу помочь? :)"

    val simpleSystemPrompt = """
        Завершай каждую свою реплику маркером $STOP_SEQUENCE, чтобы мы могли безопасно обрывать вывод. Пользователь этот маркер не увидит.
        ...
        Если пользователь ничего не говорил про квест, веди себя как обычно. Не пытайся склонить разговор в сторону квестов, просто веди светские беседы и отвечай как обычно. Даже в этом случае заканчивай ответ маркером $STOP_SEQUENCE.
    """.trimIndent()

    val questGeneratorSystemPrompt = """
        Завершай ответ маркером $STOP_SEQUENCE сразу после закрывающей фигурной скобки JSON. Маркер нужен только для остановки вывода и не должен появляться в итоговом сообщении.
    ...
    val companionSystemPrompt = """
        [Задача]
        Завершай каждую свою реплику маркером $STOP_SEQUENCE — он нужен для остановки вывода и не будет показан пользователю.
        ...
        [Финальный результат]
        Твоя цель — собрать требования, достаточные для генерации квеста в формате JSON (поля title, description, tasks). Когда деталей достаточно, сформулируй полный бриф: укажи замысел, ожидаемый результат, участников, ограничения, стиль и любые другие ключевые опоры. Этот бриф отправится генератору квестов и вернётся пользователю в виде JSON, поэтому следи за структурой и полнотой описания.
        ...
        Заверши сообщение маркером $STOP_SEQUENCE.
    """.trimIndent()
}
